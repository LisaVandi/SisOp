...
int ponteOccupato=0; /* 0 se occupato, 1 se libero */
int autoInAttesa[2]; /* auto in attesa per ogni senso di marcia */
...
{ 
	...
	*/
	...
	while(1) {
		...
		while ( (ponteOccupato) || (myBiglietto > bigliettoDistributore[indiceSenso]) || 
		...
		printf("auto %s ha finito l'attraversamento \n", Plabel);
		DBGpthread_mutex_unlock(&mutex, Plabel);
