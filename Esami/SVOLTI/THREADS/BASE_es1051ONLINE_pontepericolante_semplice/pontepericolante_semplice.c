...
int autoInAttesa[2]; /* fila che si crea in entrambi i sensi per attraversare il ponte */
int PonteOccupato=0; /* vale 0 se occupato, 1 altrimenti */
...
{ 
	...
	*/
	...
	while(1) {
		...
		DBGpthread_mutex_lock(&mutexDistributori, Plabel);
		autoInAttesa[indiceSenso]++;
		while ( (PonteOccupato) || myBiglietto > biglietto[indiceSenso] || 
				 autoInAttesa[indiceSenso] < autoInAttesa[!indiceSenso] || 
				 autoInAttesa[indiceSenso] == autoInAttesa[!indiceSenso] && indiceSenso == INDICESENSOANTIORARIO) {
			DBGpthread_cond_wait(&condPonteLibero, &mutexDistributori, Plabel);	
		}
		autoInAttesa[indiceSenso]--;
		biglietto[indiceSenso]++;
		PonteOccupato=1;/* il ponte ora è occupato */
		...
		printf("auto %s finisce l'attraversamento ponte con biglietto %i \n", Plabel, myBiglietto );
