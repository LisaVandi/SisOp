...
{ 
	...
	while(1) {
		...
		printf("ippo %s nuota verso riva sud\n", Plabel);
		fflush(stdout);
...
{ 
	...
	while(1) {
		...
		/* LISA (mie considerazioni): 
		 * ciascun ippopotamo può iniziare l'attraversamento se 
		 *  1. non c'è nessun altro ippopotamo in acqua. PRENDE LA MUTUA ESCLUSIONE
		 *  2. Fino ad un massimo di 4 lemuri possono salire in groppa all'ippopotamo 
		 */
		DBGpthread_mutex_lock(&mutexLemuri, Plabel);
		...
		DBGpthread_cond_wait(&condIppoEsceRivaSudProd, &mutexGuado, Plabel);
