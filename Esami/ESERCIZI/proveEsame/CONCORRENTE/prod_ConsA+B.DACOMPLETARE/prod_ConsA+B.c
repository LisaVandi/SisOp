/*	file:  prod_ConsA+B.c 
	...
	ma devo far eseguire deposito e prelievo in sezione critica.
		...
		- usa una variabile statoBuffer per sapere se aspetto un Prod o un ConsA o un ConsB. 
...
{ 
	...
	while(1) {
		...
		if( tipoCons==TIPOCONSA ) {
			...
			/* AGGIUNGERE CODICE */
			DBGpthread_cond_signal(&condConsB, Clabel); /* sveglio i consumatori B */
			DBGpthread_cond_wait(&condSecondo, &mutex, Clabel); /* aspetto il secondo */
