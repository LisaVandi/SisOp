...
int forchettaSx(int indice){
	return indice%NUMFILOSOFI;
}

int forchettaDx(int indice){
...
{ 
	...
	while(1) {
		...
		/* AGGIUNGERE CODICE */
		indiceforchetta1 = forchettaSx(indice);
		printf("filosofo %s vuole la forchetta di sinistra \n", Flabel ); 
		DBGpthread_mutex_lock(&mutexForchetta[indiceforchetta1], Flabel);
		forchettaoccupata[indiceforchetta1] = 1; 
		
		indiceforchetta2 = forchettaDx(indice); 
		printf("filosofo %s vuole la forchetta di destra \n", Flabel ); 
		...
		printf("filosofo %s rilascia la forchetta di destra \n", Flabel ); 
		forchettaoccupata[indiceforchetta2] = 0;

		DBGpthread_mutex_unlock(&mutexForchetta[indiceforchetta1], Flabel);
		printf("filosofo %s rilascia la forchetta di sinistra \n", Flabel ); 
