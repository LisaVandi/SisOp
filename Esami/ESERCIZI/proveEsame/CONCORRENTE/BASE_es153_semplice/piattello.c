...
{ 
	...
	while ( 1 ) {
		...
		/* quando la wait termina, il pthread ha la mutua eslusione su mutex */
		/* fine completamento */
...
{ 
	...
	*/
	...
	DBGpthread_mutex_lock(&mutex, Plabel); /* per controllare, deve guardare le variabili condivise e quindi deve detenere la mutua esclusione */
	/* fine completamento */
...
{ 
	...
	if (rc) PrintERROR_andExit(rc, "condSvegliaTiratori init has failed");
