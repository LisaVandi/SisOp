...
int autoInAttesa[2]; /* vettore per contare il numero di auto in attesa su ciascun lato */
int ponteOccupato=0; /* 0 se libero, 1 se occupato*/
...
{ 
	...
	*/
	...
	while(1) {
		...
		/* 1. verifico se il ponte è occupato e le regole di precedenza */
		autoInAttesa[indiceSenso]++;
		if (ponteOccupato || myBiglietto > biglietto[indiceSenso] || autoInAttesa[indiceSenso] < autoInAttesa[!indiceSenso]
