...
int autoInAttesa[2];
int PonteOccupato = 0; /* 0 se libero, 1 altrimenti */
...
{ 
	...
	*/
	...
	while(1) {
		...
		while ( PonteOccupato || myBiglietto > biglietto[indiceSenso] || autoInAttesa[indiceSenso] < autoInAttesa[!indiceSenso] || (autoInAttesa[indiceSenso] == autoInAttesa[!indiceSenso] && indiceSenso == INDICESENSOANTIORARIO) ) {
