...
{ 
	...
	while(1) {
		...
		DBGpthread_mutex_lock(&mutexLemuri, Plabel);
		while (LemuriPossonoSalireSuIppopotamo || NLemuriSuIppopotamo >= 4) {
			DBGpthread_cond_wait(&condIppoEntraInAcquaRivaNord, &mutexLemuri, Plabel);
		}
		LemuriPossonoSalireSuIppopotamo=1; /* ora i lemuri possono salire */
		...
		/* rilascio la mutua esclusione: i lemuri sono già avvisati nella funzione ippo di scendere */
		DBGpthread_mutex_unlock(&mutexLemuri, Plabel);
