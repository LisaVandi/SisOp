...
int numPedoniAttraversano=0, numPedoniInAttesa=0,
    autoPassa=0, numAutoInAttesa=0;
...
{ 
	...
	while(1) {
		...
		*/
		...
		while ( autoPassa!=0 || numPedoniAttraversano!=0 || numPedoniInAttesa!=0)
...
{ 
	...
	while(1) {
		...
		/* Il pedone vuole attraversare, ma lo puo' fare se e solo se
			nessuna auto sta attraversando. In caso contrario aspetta.
		*/
		while (autoPassa != 0) {
		...
		*/
		...
		else if (numPedoniAttraversano==0 && numPedoniInAttesa == 0 && numAutoInAttesa == 0) {
