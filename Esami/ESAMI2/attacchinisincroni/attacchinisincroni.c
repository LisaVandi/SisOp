...
{ 
	...
	while(1) {
		...
		*/
		...
		/* quando entrambi gli attacchini hanno smesso di lavorare
		...
		*/
		...
		DBGpthread_mutex_lock(&mutex, Alabel);
		numAttacchiniAlLavoro--;
		if (numAttacchiniAlLavoro != 0) {
			DBGpthread_cond_wait(&condAttesaFineAttacchino2, &mutex, Alabel);
		} 
		else {
